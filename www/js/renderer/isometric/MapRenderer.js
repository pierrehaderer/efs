define(["renderer/PixiContext", "renderer/Sprites", "map/Maps"], function (PixiContext, Sprites, maps) {
    ...
    class MapRenderer {
        ...
        startListen() {
            if (this.mover) {
                context.stage.removeChild(this.mover);
            } 
            this.mover = new PIXI.Container();
            context.stage.addChild(this.mover);

            /// you can click anywhere, not only on tile sprite
            this.mover.hitArea = new PIXI.Rectangle(0, 0, this.width, this.height);
            /// this means the container listen for mouse event
            this.mover.interactive = true;

            /// to store the position where you start dragging
            var drag;
            this.mover.on("mousedown", function (mouseData) {
                drag = mouseData.data.global.clone();
            });
            this.mover.on("touchstart", function (touchData) {
                drag = touchData.data.global.clone();
            });

            /// to stop dragging
            this.mover.on("mouseup", function (mouseData) {
                drag = undefined;
            });
            this.mover.on("touchend", function (touchData) {
                drag = undefined;
            });
            ...
            var move = function (mouseData) {
                if (drag) {
                    ...
                    context.stage.x = Math.max(context.renderer.width - _self.width * context.stage.scale.x, Math.min(0, context.stage.x + dx));
                    context.stage.y = Math.max(context.renderer.height - _self.height * context.stage.scale.y, Math.min(0, context.stage.y + dy));
                    drag = newPoint;
                }
            };
            this.mover.on("mousemove", move);
            this.mover.on("touchmove", move);
        };
    }
